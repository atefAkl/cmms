<?php

namespace App\Http\Controllers;

use App\Models\Compressor;
use App\Http\Requests\StoreCompressorRequest;
use App\Http\Requests\UpdateCompressorRequest;

class CompressorController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreCompressorRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(Compressor $compressor)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(Compressor $compressor)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateCompressorRequest $request, Compressor $compressor)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(Compressor $compressor)
    {
        //
    }
}
