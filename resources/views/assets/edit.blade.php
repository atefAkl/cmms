...
<div class="mb-6 flex items-center gap-4 px-4 sm:px-0">
    ...
    <div>
        <h1 class="text-2xl font-black text-gray-900">Edit Asset</h1>
        <p class="text-gray-500 text-sm">Update component details and hierarchy.</p>
    </div>
</div>
...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm font-bold text-gray-700">
                        @foreach(['active', 'inactive', 'maintenance', 'faulty'] as $status)
                ...
                <div>
                    ...
                    <select id="refrigeration_system_id" name="refrigeration_system_id" class="mt-1 block w-full border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm font-bold text-gray-700">
                        @foreach($systems as $system)
                            <option value="{{ $system->id }}" {{ old('refrigeration_system_id', $asset->refrigeration_system_id) == $system->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="parent_id" name="parent_id" class="mt-1 block w-full border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm font-bold text-gray-700">
                        ...
                        @foreach($parentAssets as $pAsset)
                            <option value="{{ $pAsset->id }}" {{ old('parent_id', $asset->parent_id) == $pAsset->id ? 'selected' : '' }}>
            ...
            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-gray-100">
                <div>
                    ...
                    <x-text-input id="manufacturer" name="manufacturer" type="text" class="mt-1 block w-full" value="{{ old('manufacturer', $asset->manufacturer) }}" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="install_date" name="install_date" type="date" class="mt-1 block w-full" value="{{ optional($asset->install_date)->format('Y-m-d') }}" />
                </div>
                <div class="md:col-span-2">
                    <x-label for="notes" value="Technical Notes" />
                    <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm font-medium text-gray-600">{{ old('notes', $asset->notes) }}</textarea>
                </div>
            </div>
        </div>

        <div class="flex items-center justify-between pt-6 border-t border-gray-100">
            <button type="button" onclick="confirm('Are you sure?') ? document.getElementById('delete-form').submit() : null" class="text-xs font-black uppercase tracking-widest text-red-500 hover:text-red-700 transition">Delete Asset</button>
    ...
    <form id="delete-form" action="{{ route('assets.destroy', $asset) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
</div>
